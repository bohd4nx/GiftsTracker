from typing import Any

from app.core.constants import EMOJIS, FOOTER
from app.utils import format_number, gift_emoji

e = EMOJIS


def create_upgrade_message_text(gift_data: dict[str, Any], username: str | None = None) -> str:
    gift_id = gift_data.get("id")

    lines = [
        f"{gift_emoji(gift_data)} <b>Gift upgrade available</b>\n",
        f"{e['gift_id']} <b>ID:</b> <code>{gift_id}</code>",
    ]

    if username:
        lines.append(f"{e['released_by']} <b>Released by:</b> @{username}")

    upgrade_price = format_number(gift_data.get("upgrade_price") or 0)
    lines.append(f"{e['upgrade_price']} <b>Upgrade Price:</b> <code>{upgrade_price}</code> {e['stars']}")

    lines.append(f"\n{FOOTER}")
    return "\n".join(lines)
